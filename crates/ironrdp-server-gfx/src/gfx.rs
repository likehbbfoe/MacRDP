...
use ironrdp_pdu::rdp::vc::dvc::gfx::{
    Avc420BitmapStream, Avc444BitmapStream, CapabilitiesConfirmPdu,
...
impl DvcProcessor for GfxHandler {
    ...
    fn process(&mut self, _channel_id: u32, payload: &[u8]) -> PduResult<Vec<DvcMessage>> {
        ...
        // Capabilities may contain newer versions than ironrdp-pdu understands.
        // Validate their framing and skip unknown versions without losing known ones.
        // The dependency's generic decoder reads the eight-byte header unchecked.
        let client_pdu: ClientPdu = if data.len() < 8 || capabilities::is_advertisement(data) {
            ClientPdu::CapabilitiesAdvertise(capabilities::decode_advertisement(data)
