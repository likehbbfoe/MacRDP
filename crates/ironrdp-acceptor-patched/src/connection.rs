    ...
    fn step(&mut self, input: &[u8], output: &mut WriteBuf) -> ConnectorResult<Written> {
        ...
        let (written, next_state) = match prev_state {
            ...
            AcceptorState::SecureSettingsExchange {
                ...
                debug!(?protocol, "Received client info (credentials omitted)");
                ...
                if !protocol.intersects(SecurityProtocol::HYBRID | SecurityProtocol::HYBRID_EX) {
                    ...
                    // TLS-only sessions have not authenticated with CredSSP, so require
                    // matching nonempty credentials. Domain differences remain ignored.
                    let creds_match = non_hybrid_credentials_match(self.creds.as_ref(), &creds);
...
fn non_hybrid_credentials_match(server: Option<&Credentials>, client: &Credentials) -> bool {
    server.is_some_and(|server| {
        !client.username.is_empty()
            && !client.password.is_empty()
